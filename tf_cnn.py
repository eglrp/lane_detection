...
# 第一层卷积+池化
w_conv1 = weight_init([5, 5, 3, 4])                             # 5x5,深度为1,16个
b_conv1 = biases_init([4])
h_conv1 = tf.nn.relu(conv2d(x_image, w_conv1) + b_conv1)    # 输出张量的尺寸:28x28x16
h_pool1 = max_pool_2x2(h_conv1)  
# 第二层卷积+池化
w_conv2 = weight_init([5, 5, 4, 8])                             # 5x5,深度为16,32个
b_conv2 = biases_init([8])
h_conv2 = tf.nn.relu(conv2d(h_pool1, w_conv2) + b_conv2)    # 输出张量的尺寸:14x14x32
h_pool2 = max_pool_2x2(h_conv2) 

# 全连接层
h_fpool2 = tf.reshape(h_pool2, [-1, 7*7*8])
# 全连接层,隐藏层节点为512个
# 权重初始化
w_fc1 = xavier_init(7*7*8, fch_nodes)
